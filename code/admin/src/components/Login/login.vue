<template>
  <div>
    <header class="banner">
      <img src="../../common/image/logo.png" class="banner-logo" alt="logo">
    </header>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapMutations(['TOKEN_CREATE']),
      login() {
        checkLogin(this.username, md5(this.password).toUpperCase()).then(res => {
          if (res.data.code) {
            this.TOKEN_CREATE(getToken())
