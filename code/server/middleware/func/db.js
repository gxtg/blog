 ...
 * @param options
 * @param populate 关联查找条件
 * @param callback
 */
export const find = (model, conditions, fields) => {
 ...
 * @param fields 查找时限定的条件，如顺序，某些字段不查找等
 * @param populate 关联查询条件
 * @param callback
 */

export const findOne = (model, conditions, fields, populate) => {
    return new Promise((resolve, reject) => {
        model.findOne(conditions, fields)
            .populate(populate)
            .exec((err, res) => {
                if (err) {
                    console.error('Error: ' + JSON.stringify(err));
 ...
 *
*/
export const findPage = async (model, conditions, fields, options={}, populate) => {
