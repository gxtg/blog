<template>
    <header class="mobile-head">
      ...
      <nav>
        <div>
          <a href="javascript:;"><i class="icon-list" @click.stop.prevent="toggleSidebar(!mobileSidebar)"></i></a>
        </div>
        <div class="name">
          <nuxt-link to="/">
            <img src="~assets/img/cat.png" alt="" width="30">
          </nuxt-link>
        </div>
        <div>
          <a href="javascript:;" @click.stop.prevent="search = !search"><i class="icon-search"></i></a>
        </div>
      </nav>
    </header>
  </template>
  ...
  export default {
    ...
    methods: {
      toggleSidebar (status) {
        this.$store.commit('options/SET_MOBILE_SIDEBAR', status)
      },
